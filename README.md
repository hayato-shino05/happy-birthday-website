...
`happy-birthday-website` は、大切な人の誕生日を「覚える」「祝う」「共有する」ためのインタラクティブな Next.js アプリケーションです。

誕生日カウントダウン、2D バースデーケーキ、メッセージ投稿、写真・動画アルバム、ミニゲーム、テーマ演出を 1 つの Web 体験にまとめています。オンラインでも、家族・友人・チームのための小さなお祝いスペースとして使えることを目指しています。
...
| アイコン | 機能 | 説明 |
|----------|------|------|
| `Timer` | リアルタイムカウントダウン | Supabase の誕生日データをもとに、次の誕生日までの残り時間を表示します。 |
| `Cake` | インタラクティブケーキ | 2D ケーキとロウソクの演出で、誕生日らしい体験を作ります。 |
| `Music` | ミュージックプレイヤー | Howler.js でバースデーソングやカスタム楽曲を再生します。 |
| `Sparkles` | ビジュアルエフェクト | 紙吹雪、花火、風船、季節ごとのパーティクル演出を表示します。 |
...
| アイコン | 機能 | 説明 |
|----------|------|------|
| `Trophy` | 神経衰弱ゲーム | カードをめくってペアを探すミニゲームです。 |
| `Puzzle` | ジグソーパズル | 任意の画像からパズルを作り、難易度を調整できます。 |
| `ClipboardList` | バースデークイズ | 誕生日に合わせたクイズを表示できます。 |
| `CalendarDays` | バースデーカレンダー | 月別に誕生日を確認できるカレンダー UI です。 |

### コミュニティ機能

| アイコン | 機能 | 説明 |
|----------|------|------|
| `MessageCircle` | リアルタイムチャット | Supabase Realtime を使ったチャット体験を提供します。 |
| `PenLine` | メッセージボード | お祝いメッセージの投稿、いいね、返信に対応します。 |
| `Mic` | ボイスメッセージ | ブラウザ上で録音し、音声メッセージとして保存できます。 |
| `Video` | ビデオメッセージ | Web カメラで動画を撮影し、ビデオメッセージとして残せます。 |
| `Gift` | バーチャルギフト | デジタルギフトを選んで送ることができます。 |
| `Users` | 共有導線 | SNS や共有ボタンから、招待 URL を届けやすくします。 |
...
| `PartyPopper` | イベントテーマ | クリスマス、ハロウィン、花見、七夕、正月などに対応します。 |
| `Video` | 動画背景 | テーマごとの動画背景とフォールバック画像を利用します。 |
| `Languages` | 多言語対応 | 日本語と英語を UI から切り替えられます。 |
...
2. **思い出を残しやすくする**
   - 写真、動画、音声、テキストメッセージをまとめて保存できます。
   - 後から見返しやすいアルバム体験を作れます。

3. **オンラインのお祝いを楽しくする**
   - ミニゲームやエフェクトで、お祝いの場に動きと遊びを加えます。
   - テーマ演出により、季節やイベントに合わせた雰囲気を出せます。
...
| 技術 | バージョン | 用途 |
|------|------------|------|
| Next.js | 16.0.7 | App Router と API Routes |
...
| `node scripts/apply-production-allowlist.mjs ... --check` | Production の clean commit から allowlist で許可された `integrationPaths` だけを確認します。 |

Production のコードを取り込む際は、allowlist の `allowedPathScopes` 内にあり、`allowedPaths` に明示され、`excludedPaths` と禁止対象（`.env`、Supabase、Storage、Realtime、deployment artifact）を除外した `integrationPaths` のみを対象にします。適用前に `--check` を実行し、現在の anonymous community と data pack の境界を維持してください。
...
<p align="center">
  <strong>特別な誕生日と、大切な人との時間のために。</strong>
</p>
