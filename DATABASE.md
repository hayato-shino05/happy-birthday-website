...
| `notification_logs` | 通知ワーカーが処理するジョブ | `event_id`, `event_type`, `recipient_ref`, `channel`, `payload`, `scheduled_at`, `next_attempt_at`, `status`, `leased_by`, `lease_until`, `attempt_count`, `last_error`, `delivered_at` |
...
- `public.consume_time_capsule_access_code(text, text)` — `service_role` 専用。招待コードの照合、ロック判定、`time_capsule_access_attempt_buckets` の更新、`time_capsules.unlock_date` 到達確認を 1 トランザクションで実行し、`time_capsule_access_codes` を返却します。
